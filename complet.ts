...
let previousPlaces: Set<string> = new Set();

// Fonction pour créer un identifiant unique d'une place
function getPlaceId(place: PlaceDisponible): string {
  return `${place.permisType}-${place.centreId}-${place.date}-${place.horaire}`;
}
...
async function notifyPlacesDisponibles(places: PlaceDisponible[]) {
  // Filtrer uniquement les nouvelles places
  const newPlaces = places.filter(place => {
    const placeId = getPlaceId(place);
    return !previousPlaces.has(placeId);
  });
  
  if (newPlaces.length === 0) {
    console.log("ℹ️  Aucune nouvelle place détectée (toutes déjà vues)");
    return;
  }
  
  console.log(`\n🆕 ${newPlaces.length} NOUVELLE(S) place(s) (sur ${places.length} total)`);
  
  const count = newPlaces.length;
  
  // Grouper par type de permis pour les SMS
  const placesByPermis = newPlaces.reduce((acc, p) => {
  ...
  const parDate = newPlaces.reduce((acc, p) => {
    if (!acc[p.date]) acc[p.date] = [];
    acc[p.date].push(p);
    return acc;
  }, {} as Record<string, PlaceDisponible[]>);
...
🔗 <a href="https://pro.permisdeconduire.gouv.fr/reserver-examen">Réserver maintenant</a>
  ...
  await Promise.all([
    ...
    sendDiscordNotification(`${count} place(s) disponible(s)`, newPlaces),
    sendTwilioNotifications(placesByPermis),
  ]);
  
  // Mettre à jour le set des places vues
  newPlaces.forEach(place => {
    previousPlaces.add(getPlaceId(place));
  });
}
  ...
  while (!isShuttingDown) {
    ...
    try {
      ...
      if (places.length > 0) {
        ...
        // Nettoyer les anciennes places qui ne sont plus disponibles
        const currentPlaceIds = new Set(places.map(getPlaceId));
        previousPlaces.forEach(placeId => {
          if (!currentPlaceIds.has(placeId)) {
            previousPlaces.delete(placeId);
          }
        });
      } else {
        console.log("\n❌ Aucune place disponible pour le moment");
        
        // Si aucune place n'est disponible, on peut nettoyer complètement
        if (previousPlaces.size > 0) {
          console.log(`ℹ️  Nettoyage de ${previousPlaces.size} place(s) précédemment vue(s)`);
